...
#  body                                                            :text
#  is_message_sent_immediately_after_last_message_from_the_same_us :boolean          default(FALSE)
#  read_at                                                         :datetime
...
class Message < ApplicationRecord
  ...
  after_create_commit :after_create_commit_action
  ...
  def after_create_commit_action
    broadcasts_new_message
  end

  def broadcasts_new_message
    # broadcast_append_later_to channel,
    #                           partial: "chat/content/receiver/message",
    #                           locals: { conversation: conversation, message: self, user: Current.user },
    #                           target: "list-conversations"
    broadcast_append_later_to channel,
                              partial: "chat/content/sender/message",
                              locals: { conversation: conversation, message: self },
                              target: "list-conversations"
  end
end
